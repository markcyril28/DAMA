...
.EXAMPLE
.\setup_conda_windows.ps1 -ValidateOnly

.EXAMPLE
.\setup_conda_windows.ps1

.EXAMPLE
.\setup_conda_windows.ps1 -Recreate -Confirm
#>
...
if ($null -eq $CondaCommand) {
    throw 'Conda was not found on PATH. Install Miniconda or open its PowerShell prompt and try again.'
}
...
$LogFile = Join-Path $LogRoot "setup_conda_windows_$LogTimestamp.txt"
...
'@
    ...
    try {
        ...
        Invoke-Conda -Description 'Environment verification' -Arguments @(
            ...
            'python', '-W', 'ignore::FutureWarning', '-c', $VerificationCode
        )
    } finally {
...
try {
    ...
    if (-not $EnvironmentExists) {
        ...
        if ($PSCmdlet.ShouldProcess("Conda environment '$EnvironmentName'", 'Ensure Python, pip, and NumPy')) {
    ...
    if ($PSCmdlet.ShouldProcess("Conda environment '$EnvironmentName'", 'Install DAMA Python dependencies')) {
        Invoke-Conda -Description 'Updating pip' -Arguments @(
    ...
    if ($PSCmdlet.ShouldProcess("Conda environment '$EnvironmentName'", "Install CUDA PyTorch from $TorchIndexUrl")) {
    ...
    Write-Host "Validate later: .\setup_conda_windows.ps1 -EnvironmentName $EnvironmentName -ValidateOnly"
